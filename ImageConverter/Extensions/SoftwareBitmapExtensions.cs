...
namespace ImageConverter.Extensions
{
    ...
    public static class SoftwareBitmapExtensions
    {
        ...
        {
            // Проверяем, что входной объект SoftwareBitmap не равен null
            if (inputBitmap == null)
            {
            ...
            SoftwareBitmap outputBitmap = new SoftwareBitmap(BitmapPixelFormat.Gray8, inputBitmap.PixelWidth, inputBitmap.PixelHeight);
                    ...
                    unsafe
                    {
                        ...
                        for (int y = 0; y < inputBitmap.PixelHeight; y++)
                        {
                            for (int x = 0; x < inputBitmap.PixelWidth; x++)
                            {
                                ...
                                int inputIndex = y * inputBitmap.PixelWidth * 4 + x * 4;
                                int outputIndex = y * outputBitmap.PixelWidth + x;

                                // Вычисляем значение оттенка серого для текущего пикселя
                                byte grayValue = (byte)((inputDataInBytes[inputIndex] + inputDataInBytes[inputIndex + 1] + inputDataInBytes[inputIndex + 2]) / 3);
        ...
        {
            // Проверяем, чтобы исходное изображение и новые размеры были валидными
            if (sourceBitmap == null || newWidth <= 0 || newHeight <= 0)
            {
                throw new ArgumentException("Invalid input parameters");
            }

            // Создаем новый экземпляр объекта SoftwareBitmap с новыми размерами
